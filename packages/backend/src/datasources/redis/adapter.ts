...
export class RedisResourceAdapter implements ResourceDataSourceAdapter {
  ...
  async deletePaths(paths: string[]): Promise<ResourceDeleteResult> {
    const keys = Array.from(new Set(paths.map((path) => path.trim()).filter(Boolean)))
    if (!keys.length) {
      return {
        deletedPaths: [],
        deletedCount: 0,
  ...
  private async listKeys(options?: ResourceListOptions): Promise<ResourceBrowserListing> {
    const limit = Math.max(options?.limit ?? 200, 1)
    const keys: string[] = []
    let cursor = options?.cursor ?? '0'
    const matchPattern = options?.search?.trim() ? `*${options.search.trim()}*` : '*'
  ...
  private async getKey(key: string): Promise<ResourceBrowserListing> {
    ...
    if (keyType === 'string') {
      ...
      preview = {
        type: 'text',
        title: `String • ${key}`,
        text: typeof stringValue === 'string' ? stringValue : String(stringValue ?? ''),
      }
