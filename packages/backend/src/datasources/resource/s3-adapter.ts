...
import { buildS3ObjectDetails, buildS3ObjectPreview, getS3ContentType, getS3PreviewByteLimit } from './s3-preview.ts'
import { getS3ParentPath, getS3ResourceName, parseS3ResourcePath } from './s3-paths.ts'
import type { ResourceDataSourceAdapter, ResourceDeleteResult, ResourceListOptions } from './types.ts'
...
export class S3ResourceAdapter implements ResourceDataSourceAdapter {
  ...
  private async deletePath(path: string) {
    ...
    if (!parsed.objectKey) {
      const keys = await this.listObjectKeys(parsed.bucket)
      await this.removeObjectKeys(parsed.bucket, keys)
      await this.client.removeBucket(parsed.bucket)
      return
    }
    ...
    const normalizedPrefix = parsed.objectKey.replace(/\/+$/, '')
    const keys = await this.listObjectKeys(parsed.bucket, `${normalizedPrefix}/`)
    const exactFolderObject = await this.getExistingObjectKey(parsed.bucket, normalizedPrefix)
    if (exactFolderObject) {
      keys.unshift(exactFolderObject)
    }

    await this.removeObjectKeys(parsed.bucket, Array.from(new Set(keys)))
  }

  private async listObjectKeys(bucket: string, prefix = '') {
    const keys: string[] = []
    let startAfter = ''

    while (true) {
      const response = await this.client.listObjectsV2Query(bucket, prefix, '', '', 1000, startAfter)
      const pageKeys = response.objects
  ...
  private async getExistingObjectKey(bucket: string, key: string) {
    try {
  ...
  private async removeObjectKeys(bucket: string, keys: string[]) {
    if (!keys.length) {
      return
    }
    ...
    for (let index = 0; index < keys.length; index += DELETE_BATCH_SIZE) {
      ...
      const responses = await this.client.removeObjects(bucket, batch)
      const errorResponse = responses.find((entry) => entry?.Error)
