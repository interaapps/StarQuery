...
export const mssqlSqlDialect = defineSqlDialect({
  type: 'mssql',
  ...
  getDefaultSchemaName(source) {
    return getTrimmedString(source.config.schema) ?? 'dbo'
  },
  ...
  getTableSchemaUi(mode) {
    return {
      editDescription:
        mode === 'create'
          ? 'Create a new table with columns, keys, foreign keys, checks, and indexes. Editing existing SQL Server tables in the schema designer is still disabled for now.'
          : 'Edit existing SQL Server tables column-by-column. More advanced constraint editing is still best done through SQL for now.',
